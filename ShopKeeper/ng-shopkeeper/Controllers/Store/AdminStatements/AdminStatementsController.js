...
define(['application-configuration', 'salesReportServices'], function (app)
{
    ...
    function ($scope, $rootScope, $routeParams, salesReportServices, $filter, $locale)
    {
        ...
        $scope.getSupplierStatements = function (supplier)
        {
               ...
               angular.element('#SupplierSummary').emppty();
        ...
        $scope.GetCustomerStatements = function (customer)
        {
            ...
            if (customer !== undefined && customer !== null && customer.CustomerId > 0)
            {
             ...
             console.log(JSON.stringify(selCustomer));
        ...
        $scope.retrieveCustomersStatement = function (payload)
        {
            ...
            {
                url = '/Report/GetAllCustomerStatements?itemsPerPage=' + payload.itemsPerPage + '&pageNumber=' + payload.pageNumber;
            }
            else
            {
                $scope.repCustomer = payload.customer;
                url = '/Report/GetSingleCustomerInvoiceStatements?customerId=' + payload.customer.CustomerId + '&itemsPerPage=' + payload.itemsPerPage + '&pageNumber=' + payload.pageNumber;
            }
        ...
        $scope.getCustomerStatementCompleted = function (data)
        {
            ...
            if (data.length < 1)
            {
                if ($scope.customerPayload.totalPaid > 0)
                {
                    ...
                    {
                        $scope.header += '<tr style="font-size:0.9em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000">Transactions Statement for Customer: <b>' + $scope.customerPayload.customer.UserProfileName + '</b></h5></td><td style="width: 20%"></td></tr>';
                    }
                    else
                    {
                        $scope.header += '<tr style="font-size:0.9em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000">Transactions Statement for all Customers</h5></td><td style="width: 20%"></td></tr>';
