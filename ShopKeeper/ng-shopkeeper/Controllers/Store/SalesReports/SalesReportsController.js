    ...
    function ($scope, $rootScope, $routeParams, salesReportServices, $filter, $locale)
    {
        ...
        $scope.initializeController = function () 
        {
            ...
            [
                ...
                { name: 'Product Category', typeId: 6 }
            ];
        ...
        $scope.checkReportType = function ()
        {
            ...
            {
                $scope.showCatSearch = true;
                $rootScope.getCategoryReports(null);
            }
        };
        ...
        $scope.getDefaultsCompleted = function (data)
        {
            ...
            $scope.categories = data.ItemCategories;
        ...
        $rootScope.getCategoryReports = function (selectedCategory) {
            if ($scope.startDate === undefined || $scope.startDate == null || $scope.startDate.length < 1) {
            ...
            var category =
            {
                StoreItemCategoryId: 0
            };

            if (selectedCategory !== undefined && selectedCategory !== null && selectedCategory.StoreItemCategoryId > 0)
            {
            ...
            $scope.ctReport = false;

            angular.element('#categorySummary').empty();
            angular.element('#categoryTbl tbody').empty();
            $scope.categoryCollection = [];
            $scope.retrieveCategoryReport($scope.categoryLoad);
        };
        
        $scope.retrieveCategoryReport = function (payload)
        {
            var url = '';

            if (payload.category === undefined || payload.category == null || payload.category.StoreItemCategoryId < 1)
            {
                url = '/Report/GetAllSalesReportByCategory?startDateStr=' + payload.startDate + '&endDateStr=' + payload.endDate + '&itemsPerPage=' + payload.itemsPerPage + '&pageNumber=' + payload.pageNumber;
            }
            else
            {
                $rootScope.category = payload.category;

                url = '/Report/GetSalesReportByCategory?categoryId=' + payload.category.StoreItemCategoryId + '&startDateStr=' + payload.startDate + '&endDateStr=' + payload.endDate + '&itemsPerPage=' + payload.itemsPerPage + '&pageNumber=' + payload.pageNumber;
            }

            $scope.processing = true;
            salesReportServices.getReports(url, $scope.getCategoryReportsCompleted);
        }
        
        $scope.getCategoryReportsCompleted = function (data)
        {
            $scope.processing = false;

            if (data.length < 1)
            {
                if ($scope.categoryLoad.totalPaid > 0)
                {
                    var totalPaid = filterCurrency($scope.categoryLoad.totalPaid, '');
                    var totalCost = filterCurrency($scope.categoryLoad.totalCost, '');
                    var totalProfit = filterCurrency($scope.categoryLoad.totalProfit, '');
                    var totalNet = filterCurrency($scope.categoryLoad.totalNet, '');
                    ...
                    if ($scope.categoryLoad.category.StoreItemCategoryId > 0)
                    {
                        $scope.header += '<tr style="font-size:0.71em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000">Sales Report for <b>' + $scope.category.Name + '</b> within the period <b>' + $scope.categoryLoad.strDt + '-' + $scope.categoryLoad.endDt + '</b></h5></td><td style="width: 20%"></td></tr>';
                    }
                    else
                    {
                        $scope.header += '<tr style="font-size:0.71em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000">Sales Report for all Product categories within the period <b>' + $scope.categoryLoad.strDt + '-' + $scope.categoryLoad.endDt + '</b></h5></td><td style="width: 20%"></td></tr>';
                    ...
                    var subHeader = '<div class="row" style=" margin-top: 2%"><b>SUMMARY:</b></div><table style="border: solid 1px #ddd; width: 100%; margin-bottom: 2%"><tr style="border-bottom: solid 1px #ddd; font-size:0.85em; font-weight: bold;"><td style="width: 20%">' +
                        'TOTAL NET SALES(' + $rootScope.store.DefaultCurrencySymbol + ')</td><td style="width: 20%;">TOTAL PAYMENT(' + $rootScope.store.DefaultCurrencySymbol + ')</td><td style="width: 20%;">TOTAL COST OF SALES(' + $rootScope.store.DefaultCurrencySymbol + ')</td><td style="width: 20%;">TOTAL MARGIN(' + $rootScope.store.DefaultCurrencySymbol + ')</td></tr>' +
                        '<tr style="font-size:0.79em; font-weight: bold"><td style="width: 20%">' + totalNet + '</td>' +
                        '<td style="width: 20%;">' + totalPaid + '</td><td style="width: 20%;">' + totalCost + '</td><td style="width: 20%;">' + totalProfit + '</td></tr></table>';

                    $scope.header += subHeader.replace('0.85em', '0.7em').replace('0.79em', '0.65em').replace('0.71em', '0.69em');
                    angular.element('#categorySummary').html('').append(subHeader);
                }
           ...
           var rows = '';
            ...
            {
                $scope.categoryCollection.push(r);
                
                $scope.categoryLoad.totalProfit += r.Profit;
                $scope.categoryLoad.totalCost += r.CostOfSale;
                $scope.categoryLoad.totalNet += r.NetAmount;
                $scope.categoryLoad.totalPaid += r.AmountPaid;

                rows += '<tr role="row" class="odd" style="border-top:solid 1px #ddd;"><td  style="width: 3%;">' + $scope.categoryLoad.sn + '</td><td>' + r.CategoryName + '</td><td>' + filterCurrency(r.NetAmount, " ") + '</td><td>' + filterCurrency(r.AmountPaid, " ")
                    + '</td><td>' + filterCurrency(r.CostOfSale, " ") + '</td><td>' + filterCurrency(r.Profit, " ") + '</td></tr>';
                $scope.categoryLoad.sn++;
            });
            
            angular.element('#categoryTbl > tbody:last-child').append($scope.compiler(rows)($scope));
            $scope.categoryLoad.itemsPerPage = 100;
            $scope.categoryLoad.pageNumber += 1;
            $scope.retrieveCategoryReport($scope.categoryLoad);
        };
        ...
        $rootScope.getOutletReports = function (outlet)
        {
            ...
            angular.element('#outletSummary').show();
        ...
        $rootScope.getPaymentTypeReports = function (selectedPaymentType)
        {
            ...
            angular.element('#pyTypeSummary').html('');
        ...
        $scope.getPaymetReportsCompleted = function (data)
        {
            ...
            if (data.length < 1)
            {
                if ($scope.ptyPayload.totalAmount > 0)
                {
                    ...
                    {
                        $scope.header += '<tr style="font-size:0.9em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000"><b>' + $scope.ptyPayload.paymentType.Name + ' </b> Payments made within the period <b>' + $scope.ptyPayload.strDt + '-' + $scope.ptyPayload.endDt + '</b></h5></td><td style="width: 20%"></td></tr>';
                    }
                    else
                    {
                        $scope.header += '<tr style="font-size:0.9em;"><td style="width: 20%"></td><td  style="width: 60%;"><h5 style="border-bottom: 1px solid #000">Report for all Payment Methods within the period <b>' + $scope.ptyPayload.strDt + '-' + $scope.ptyPayload.endDt + '</b></h5></td><td style="width: 20%"></td></tr>';
                    ...
                    var subHeader = '<div class="row" style=" margin-top: 2%"><b>SUMMARY:</b></div>' +
                        '<table style="width: 100%; margin-bottom: 2%"><tr style="font-size:0.84em; font-weight: bold;"><td style="width: 20%">' +
                        'TOTAL TRANSACTIONS</td><td style="width: 20%;">TOTAL TRANSACTIONS VALUE(' + $rootScope.store.DefaultCurrencySymbol + ')</td><td style="width: 20%;">TOTAL REFUNDED AMOUNT (' + $rootScope.store.DefaultCurrencySymbol + ')</td><td style="width: 20%;">TOTAL AVAILABLE AMOUNT(' + $rootScope.store.DefaultCurrencySymbol + ')</td></tr><tr style="font-size:0.85em; font-weight: bold;">' +
                        '<td style="width: 20%;">' + $scope.ptyPayload.totalcount + '</td>'
                        + '<td style="width: 20%;">' + filterCurrency($scope.ptyPayload.totalAmount, " ") + '</td><td style="width: 20%;">' + filterCurrency($scope.ptyPayload.totalRefundedAmount, " ") + '</td><td style="width: 20%;">' + filterCurrency($scope.ptyPayload.totalAvailableAmount, " ") + '</td></tr></table>';
                    
                    $scope.header += subHeader.replace('0.84em', '0.7em').replace('0.85em', '0.65em');
                    ...
                    angular.forEach($scope.ptyCollection, function (r, i)
                    {
                        rows += '<tr role="row" class="odd" style="border-top:solid 1px #ddd;"><td  style="width: 3%;">'
                            + sn + '</td><td>' + r.Name + '</td><td>' + r.TotalTransactions + '</td><td>' + filterCurrency(r.TotalTransactionValue, " ") + '</td><td>' + filterCurrency(r.TotalRefundedAmount, " ") + '</td><td>' + filterCurrency(r.TotalAvailableAmount, " ") + '</td></tr>';
                        sn++;
                        $scope.ptyPayload.sn++;
                    });
            ...
            angular.forEach(data, function (r, i)
            {
                ...
                if (tt.length > 0)
                {
                    ...
                    f.TotalRefundedAmount += r.TotalRefundedAmount;
                    f.TotalAvailableAmount += r.TotalAvailableAmount;
                }
                else
                {
                ...
                $scope.ptyPayload.totalAvailableAmount += r.TotalAvailableAmount;
                $scope.ptyPayload.totalRefundedAmount += r.TotalRefundedAmount;
            });
        ...
        $rootScope.getProductReports = function (selectedProduct)
        {
            ...
            angular.element('#prSummary').empty();
        ...
        $scope.getProductReportsCompleted = function (data)
        {
            ...
            if (data.length < 1)
            {
                if ($scope.prPayload.totalAmountSold > 0)
                {
                    ...
                    angular.forEach($scope.prCollection, function (r, i)
                    {
                    ...
                    var totalCostOfSale = filterCurrency($scope.prPayload.totalCostOfSale);
                    ...
                    var subHeader = '<div class="row" style=" margin-top: 2%"><b>SUMMARY:</b></div><table style="border: solid 1px #ddd; width: 100%; margin-bottom: 2%"><tr style="border-bottom: solid 1px #ddd; font-size:0.9em; font-weight: bold;"><td style="width: 20%">' +
                        ...
                        '</td><td style="width: 20%">TOTAL COST OF SALE</td></tr>' +
                        '<tr style="font-size:0.85em; font-weight: bold"><td style="width: 20%">' + totalQuantitySold + '</td><td style="width: 20%;">' + totalAmountSold + '</td><td style="width: 20%">' + totalCostOfSale + '</td>' +
                        '</tr></table>';
            ...
            angular.forEach(data, function (r, i)
            {
                ...
                $scope.prPayload.totalAmountSold += r.AmountSold;
                $scope.prPayload.totalQuantitySold += r.QuantitySold;
                $scope.prPayload.totalCostOfSale += r.CostPrice * r.QuantitySold;
                ...
                rows += '<tr role="row" class="odd" style="border-top:solid 1px #ddd;"><td  style="width: 3%;">' + $scope.prPayload.sn + '</td><td>' + r.InvoiceNumber + '</td><td>' + r.DateSoldStr + '</td><td>' + r.Sku + '</td><td>' + r.StoreItemName + '</td><td style="text-align : right">' + r.QuantitySoldStr + '</td><td style="text-align : right">' + r.RateStr + '</td><td style="text-align : right">' + r.AmountSoldStr + '</td><td style="text-align : right">' + filterCurrency(r.CostPrice * r.QuantitySold, '') + '</td></tr>';
                $scope.prPayload.sn++;
            });
        ...
        $rootScope.getEmployeeReports = function (selectedEmployee)
        {
            ...
            angular.element('#empSummary').html('');
        ...
        $scope.getCustomerReportsCompleted = function (data)
        {
            ...
            if (data.length < 1)
            {
                if ($scope.customerPayload.totalPaid > 0)
                {
                    ...
                    var subHeader = '<div class="row" style=" margin-top: 2%"><b>SUMMARY:</b></div><table style="border: solid 1px #ddd; width: 100%; margin-bottom: 2%"><tr style="border-bottom: solid 1px #ddd; font-size:0.85em; font-weight: bold;"><td style="width: 20%">' +
                        'TOTAL NET</td><td style="width: 20%;">TOTAL AMOUNT PAID</td></tr>' +
                        '<tr style="font-size:0.79em; font-weight: bold"><td style="width: 20%">' + totalDue + '</td><td style="width: 20%;">' + totalPaid + '</td></tr></table>';
        ...
        $scope.buildSingleCustomerRepTable = function (data) {
            ...
            angular.forEach(data, function (r, i)
            {
                ...
                var template = '<td style="width: 5%; text-align:center"><a title="Get Report Details" id="' + r[0] + '" style="cursor: pointer" ng-click = "' + 'getSalesReport(' + r[0] + ')"><img src="/Content/images/details.png" /></a></td>';
                var row = '<tr role="row" class="odd" style="border-top:solid 1px #ddd;"><td  style="width: 3%;">' + sn + '</td><td>' + r[1] + '</td><td>' + r[2] + '</td><td>' + r[4] + '</td><td>' + r[5] + '</td><td>' + r[6] + '</td><td>' + r[7] + '</td><td>' + r[8] + '</td>' + template + '</tr>';
        ...
        $scope.buildMultipleCustomerRepTable = function (data) {
            ...
            angular.forEach(data, function (r, i)
            {
                ...
                var template = '<td style="width: 5%; text-align:center"><a title="Get Report Details" id="' + r[0] + '" style="cursor: pointer" ng-click = "' + 'getSalesReport(' + r[0] + ')"><img src="/Content/images/details.png" /></a></td>';
                var row = '<tr role="row" style="border-top:solid 1px #ddd;"><td  style="width: 3%;">' + sn + '</td><td>' + r[1] + '</td><td>' + r[2] + '</td><td>' + r[3] + '</td><td>' + r[4] + '</td><td>' + r[5] + '</td><td>' + r[6] + '</td><td>' + r[7] + '</td><td>' + r[8] + '</td><td>' + r[9] + '</td>' + template + '</tr>';
        ...
        $scope.getSalesReportDetailsCompleted = function (rec)
        {
         ...
         $scope.details = true;
         salesReportServices.getSaleRefundNotes(rec.SaleId, $scope.getSaleRefundNotesCompleted);
        ...
        $scope.printCategoryReport = function ()
        {
            var items = $scope.categoryCollection;
            ...
            var sr = $scope.header + '<table style="border: solid 1px #ddd; width:100%"><thead><tr style="color: #000; border-top: solid 1px #ddd; font-size:0.65em;height:32px">' +
                '<th style="width: 3%; text-align: left">S/N</th><th style="width: 17%; text-align: left">Product Category</th><th style="width: 12%; text-align: right">Net Sales(' + $rootScope.store.DefaultCurrencySymbol + ')</th>' +
                '<th style="width: 10%; text-align: right">Payment(' + $rootScope.store.DefaultCurrencySymbol + ')</th>' +
                '<th style="width: 12%; text-align: right">Cost of Sales(' + $rootScope.store.DefaultCurrencySymbol + ')</th><th style="width: 12%; text-align: right">' +
                'Margin(' + $rootScope.store.DefaultCurrencySymbol + ')</th></tr></thead><tbody>';
            
            var sn = 1;

            angular.forEach(items, function (r, i)
            {
                sr += '<tr role="row" class="odd" style="border-top:solid 1px #ddd; font-size:0.62em;"><td>' + sn + '</td><td>' + r.CategoryName + '</td><td style="text-align: right">' + filterCurrency(r.NetAmount, " ") + '</td><td style="text-align: right">' + filterCurrency(r.AmountPaid, " ")
                + '</td><td style="text-align: right">' + filterCurrency(r.CostOfSale, " ") + '</td><td style="text-align: right">' + filterCurrency(r.Profit, " ") + '</td></tr>';
                sn++;
            });
        ...
        $scope.printProductReport = function (collection)
        {
            ...
            var sr = $scope.header + '<table style="border: solid 1px #ddd; width:100%"><thead><tr style="color: #000; border-top: solid 1px #ddd; font-size:0.62em;height:32px;"><th style="width: 3%; text-align: left">S/N</th><th style="width: 12%; text-align: left">' +
                ...
                'Amount(' + $rootScope.store.DefaultCurrencySymbol + ')</th><th style="width: 10%; text-align: left">Cost Of Sales</th></tr></thead><tbody>';

            var sn = 1;
            angular.forEach(items, function (r, i)
            {
                sr += '<tr style="border-top:solid 1px #ddd; font-size:0.6em;"><td  style="width: 3%;">' + $scope.prPayload.sn + '</td><td>' + r.InvoiceNumber + '</td><td>' + r.DateSoldStr + '</td><td>' + r.Sku + '</td><td>' + r.StoreItemName + '</td><td>' + r.QuantitySoldStr + '</td><td>' + r.RateStr + '</td><td>' + r.AmountSoldStr + '</td><td>' + r.CostOfSaleStr + '</td></tr>';
                sn++;
            });
        ...
        $scope.printPaymentTypeReport = function (collection)
        {
            ...
            var sr = $scope.header + '<table style="border: solid 1px #ddd; width:100%"><thead><tr  style="color: #000; border-top: solid 1px #ddd; font-size:0.7em;height:32px;"><th style="width: 3%; text-align: left">S/N</th><th style="width: 15%; text-align: left">' +
            'Payment Method</th><th style="width: 15%; text-align: left">Number of Transactions</th><th style="width: 16%; text-align: left">' +
            'Transaction Value(' + $rootScope.store.DefaultCurrencySymbol + ')</th><th style="width: 13%; text-align: left">Amount Refunded(' + $rootScope.store.DefaultCurrencySymbol + ')</th><th style="width: 13%; text-align: left">Available Amount(' + $rootScope.store.DefaultCurrencySymbol + ')</th></tr></thead><tbody>';

            var sn = 1;
            angular.forEach($scope.ptyCollection, function (r, i)
            {
                sr += '<tr role="row" class="odd" style="border-top:solid 1px #ddd; font-size:0.68em"><td  style="width: 3%;">'
                    + sn + '</td><td>' + r.Name + '</td><td>' + r.TotalTransactions + '</td><td>' + filterCurrency(r.TotalTransactionValue, " ") + '</td><td>' + filterCurrency(r.TotalRefundedAmount, " ") + '</td><td>' + filterCurrency(r.TotalAvailableAmount, " ") + '</td></tr>';
                sn++;
            });
